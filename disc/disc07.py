...
class Button:
    ...
    def press(self):
        ...
        self.pressed += 1
        "*** YOUR CODE HERE ***"
        if isinstance(self, CapsLock):
            self.caps_lock.press()
        else:
...
class Keyboard:
    ...
    def __init__(self):
        self.typed = []
        self.keys = {letter: Button(letter, lambda x: self.typed.append(x)) for letter in LOWERCASE_LETTERS}# Try a dictionary comprehension!
